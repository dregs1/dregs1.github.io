...
import gzip
import six
from six.moves.urllib.request import urlopen, Request, build_opener, HTTPHandler, HTTPCookieProcessor, install_opener, HTTPRedirectHandler, urlretrieve
    ...
    # one of the two cookie handling modules
        ...
        if cookielib is not None:
            ...
            if not follow_redirects:
                ...
                opener = build_opener(HTTPHandler(debuglevel=http_debug_log_enabled),HTTPCookieProcessor(cj),NoRedirectHandler())

            else:

                opener = build_opener(HTTPHandler(debuglevel=http_debug_log_enabled),HTTPCookieProcessor(cj))
...
def get_temp_path():
    ...
    dev = translatePath( "special://temp/" )
...
def get_runtime_path():
    ...
    dev = translatePath( __settings__.getAddonInfo('Path') )
